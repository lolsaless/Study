...
label_data <- subset(data, Ca >= 40)  # Replace 'your_threshold' with your specific threshold for Ca

# Calculating maximum values for each product or category
max_values <- data %>%
    group_by(Products) %>%
    summarise(max_Ca = max(Ca))

# Create a label for maximum values
max_values$label <- paste(max_values$max_Ca)

# Base plot
p <- ggplot(data, aes(x = Products , y = Ca, fill = Products)) +
    stat_boxplot(geom = "errorbar", width = 0.25) +
    geom_boxplot(outlier.shape = NA) +
    geom_point(data = subset(data, Ca > 20), aes(x = Products, y = Ca), color = "red", shape = 1) +
    ...
    geom_text(data = max_values, aes(x = Products, y = max_Ca, label = label), vjust = -1.5, color = "blue")

p

# Rest of the plot customization (adjust according to your needs)
p + coord_cartesian(ylim = c(0, 115)) +
