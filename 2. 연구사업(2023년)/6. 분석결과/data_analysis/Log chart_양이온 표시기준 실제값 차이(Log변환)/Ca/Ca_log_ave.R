...
library(ggrepel)
library(scales)
setwd("D:/github/Study/2. 연구사업(2023년)/6. 분석결과/data_analysis/Log chart_양이온 표시기준 실제값 차이(Log변환)/Ca")
...
## Data Transformation
# Transforming Ca
df$Transformed_Ca <- df$Ca - df$Ca_ave

# Log Transformation
df$Log_Ca_max <- log10(df$Ca_ave)
...
p <- ggplot(df, aes(x=row_num, y=Transformed_Log_Ca, color=Level, shape=Sample)) +
    ...
    labs(title="Transformed Log Ca Values", x="Index", y="Log-transformed Ca") +
    theme_minimal() +
    scale_colour_manual(values = c("Low" = "#2E8B57", "Medium" = "#FF8C00", "High" = "#D2691E", "Very High" = "#DC143C"))
...
p <- ggplot(df, aes(x=row_num, y=Transformed_Log_Ca, color=Level, fill=Level, shape=Sample)) +
    ...
    labs(x="Index", y="Log-transformed Ca") + # title = "ca" 삭제함.
    theme_minimal() +
    theme(
