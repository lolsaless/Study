...
setwd("D:/github/Study/2. 연구사업(2023년)/6. 분석결과/data_analysis/Log chart_양이온 표시기준 실제값 차이(Log변환)/K")
...
## Data Transformation
# Transforming K
df$Transformed_K <- df$K - df$K_ave

# Log Transformation
df$Log_K_max <- log10(df$K_ave)
df$Log_K <- log10(df$K)
...
df$Level <- cut(df$Transformed_Log_K, 
                breaks = c(-Inf, -1.5, -1, -0.5, 0.5, 1, Inf), 
                labels = c("Very High", "High", "Medium", "Low", "Medium", "High"))

# ggplot 코드
p <- ggplot(df, aes(x=row_num, y=Transformed_Log_K, color=Level, shape=Sample)) +
    geom_point() +
    scale_shape_manual(values = c(2, 1)) +
    geom_hline(yintercept=0, linetype="dashed", color = "red") +
    geom_hline(yintercept=c(0.5, -0.5), linetype="dashed", color = "#4682B4") +
    labs(title="Transformed Log K Values", x="sample", y="Log K") +
    theme_minimal() +
    scale_colour_manual(values = c("Low" = "#2E8B57", "Medium" = "#FF8C00", "High" = "#D2691E", "Very High" = "#DC143C"))
...
p <- ggplot(df, aes(x=row_num, y=Transformed_Log_K, color=Level, fill=Level, shape=Sample)) +
    ...
    labs(x="Index", y="Log-transformed K") +
    theme_minimal() +
    theme(
