...
label_data <- subset(data, Na >= 15)  # Replace 'your_threshold' with your specific threshold for Na

# Calculating maximum values for each product or category
max_values <- data %>%
    group_by(Products) %>%
    summarise(max_Na = max(Na))

# Create a label for maximum values
max_values$label <- paste(max_values$max_Na)

# Base plot
p <- ggplot(data, aes(x = Products , y = Na, fill = Products)) +
    stat_boxplot(geom = "errorbar", width = 0.15) +
    geom_boxplot(outlier.shape = NA) +
    geom_point(data = subset(data, Na > 15), aes(x = Products, y = Na), color = "red", shape = 1) +
    ...
    geom_text(data = max_values, aes(x = Products, y = max_Na, label = label), vjust = -1.5, color = "blue")
