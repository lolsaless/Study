...
label_data <- subset(data, K >= 5)  # Replace 'your_threshold' with your specific threshold for Ca
...
p <- ggplot(data, aes(x = Products , y = K, fill = Products)) +
    ...
    geom_point(data = subset(data, K > 4), aes(x = Products, y = K), color = "red", shape = 1) +
